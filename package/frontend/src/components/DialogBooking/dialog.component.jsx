...
const DialogBookingScreen = ({ openformLogin, BackOpenformLogin, movie, movies }) => {
  ...
  const setIndexCineplex = (idx, value) => {
    for (let i = 0; i < valueCineplex.length; i++) {
      const result = valueCineplex[i].details.find((el) => el.cineplex == value);
  ...
  const setIndexCategory = (idx, value) => {
    for (let i = 0; i < category.length; i++) {
      const result = category.find((el) => el.cate_room == value);
      const newColor = new Array(valueCineplex.length).fill(null);
      newColor[idx] = "changeColorBackground";
  ...
  return (
    ...
    >
      <div className="bookingscreen">
          ...
          <>
            <div className="theater-cluster">
              {valueCineplex[0].details.length > 0 ? (
                valueCineplex[0].details.map((dt, idx) => (
                  <button
                    className={`btncinema-complex ${colorCineplex[idx]}`}
                    onClick={() => setIndexCineplex(idx, dt.cineplex)}
                  >
                    <div>{dt.cineplex}</div>
                  </button>
                ))
            ...
            <div className="movie-genre ">
              {category?.map((dtc, idx) => (
                <button
                  className={`btncategory ${colorCategory[idx]}`}
        ...
        {time && category && valueCineplex && valueDay ? (
